$.fn.recurring_select = function(method) {
    var methods = Object.getOwnPropertyNames(RecurringSelect.prototype);
...
RecurringSelect = class RecurringSelect {

    // I see what's going on, the original code assumed "methods" were dynamically attached to a jQuery object
    // By componentizing the code it now assumes the instance is "this" rather than the jQuery object which is "this"

    constructor(recurring_selector) {
        var self = this;
        $(document).on("focus", ".recurring_select", function() {
            return self.set_initial_values();
        });
